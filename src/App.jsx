...
function PlannedDetail({user,p,onClose,onSave,onDelete,onFlash,onPhoto}){
  ...
  const[showRepl,setShowRepl]=useState(false);const[repl,setRepl]=useState(p.pieceReplaced||"");
  ...
  const saveReplaced=()=>{if(!repl.trim())return;onSave({...p,pieceReplaced:repl.trim(),pieceReplacedBy:user.name,pieceReplacedAt:Date.now()});setShowRepl(false);onFlash("Pezzo sostituito registrato ✓");};
  ...
  return(
    <Sheet onClose={onClose} title={"Camera "+p.room+" · Intervento"}>
      ...
      {p.pieceReplaced&&blk("#F5F3FF","#DDD6FE",<>{dlbl("Pezzo sostituito","#6D28D9")}<div style={{fontSize:14,lineHeight:1.4,marginBottom:6}}>{p.pieceReplaced}</div><div style={{fontSize:11,color:"#5C645E"}}>Da {p.pieceReplacedBy} · {fmt(p.pieceReplacedAt)}</div></>)}
      ...
      <button onClick={()=>setShowRepl(true)} style={{...ctaSt,background:"#0E5C49",marginBottom:10}}>{I.pkg} Pezzo sostituito</button>
      )}
      {canOrderPiece&&showRepl&&(
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:10}}>
      <input style={inputSt} placeholder="Cosa hai sostituito..." value={repl} onChange={e=>setRepl(e.target.value)} autoFocus/>
      <div style={{display:"flex",gap:8}}>
      <button onClick={saveReplaced} disabled={!repl.trim()} style={{...ctaSt,background:"#0E5C49",flex:1,opacity:repl.trim()?1:.5}}>{I.check} Conferma</button>
      <button onClick={()=>setShowRepl(false)} style={{...ctaSt,background:"#E4E0D6",color:"#1B2420",flex:"0 0 44px"}}>{I.x}</button>
      </div>
      </div>
      )}
...
function Detail({user,it,tec,onClose,onPhoto,onSave,onDelete,onFlash}){
  ...
  const[showW,setShowW]=useState(false);const[showT,setShowT]=useState(false);const[showRepl,setShowRepl]=useState(false);const[repl,setRepl]=useState(it.pieceReplaced||"");
  ...
  const pieceArr=()=>{onSave({...it,status:"todo",pieceArrivedAt:Date.now()});onClose();onFlash("Torna in Da fare ✓");};const saveReplaced=()=>{if(!repl.trim())return;onSave({...it,pieceReplaced:repl.trim(),pieceReplacedBy:user.name,pieceReplacedAt:Date.now()});setShowRepl(false);onFlash("Pezzo sostituito registrato ✓");};
